...
const exportQualityRow = document.getElementById("exportQualityRow");
...
const views = {
  ...
  bgremove: document.getElementById("view-bgremove"),
...
const exportRatioLock = document.getElementById("exportRatioLock");
const exportRatioWarning = document.getElementById("exportRatioWarning");
...
let syncingExportCustomSize = false;
...
function getSessionCrops() {
  return generatedCrops.filter((item) => sessionCropIds.has(item.id));
}

function resizeCanvasBufferToDisplaySize(canvas) {
...
function syncControls() {
  ...
  if (saveMeta) saveMeta.textContent = `${getSessionCrops().length}개`;
  updateWorksBadge();
  updateExportInfo();
...
function loadFile(file, readyMessage) {
  if (!file || !file.type.startsWith("image/")) {
    setStatus("이미지 파일만 사용할 수 있어요");
    return;
  }
  if (isGifFile(file)) {
    setStatus("GIF 애니메이션은 유지되지 않아 PNG/JPG/WebP를 사용해 주세요");
    return;
  }
  ...
  image.onload = () => {
    ...
    syncControls();
    resetHistory();
    setStatus(readyMessage || "도형을 골라 이미지 위에서 드래그해 그리세요");
  };
  image.onerror = () => setStatus("이미지를 읽지 못했어요");
  image.src = sourceObjectUrl;
}

function useFirstImageFile(fileList, sourceLabel = "파일") {
  ...
  const usableFiles = imageFiles.filter((file) => !isGifFile(file));
  if (!usableFiles.length) {
    setStatus("GIF 애니메이션은 유지되지 않아 PNG/JPG/WebP를 사용해 주세요");
    return;
  }
  const skippedGifs = imageFiles.length - usableFiles.length;
  const readyMessage = usableFiles.length > 1 || skippedGifs
    ? `${sourceLabel} ${imageFiles.length}개 중 첫 번째 지원 이미지만 사용했어요${skippedGifs ? ` · GIF ${skippedGifs}개 제외` : ""}`
    : undefined;
  loadFile(usableFiles[0], readyMessage);
}

function resetResults() {
  if (!getSessionCrops().length) return;
  if (!window.confirm("현재 화면의 결과 목록만 비울까요? 내 작업 보관함은 유지됩니다.")) return;
  sessionCropIds.clear();
  renderWorks();
  setStatus("현재 결과 목록을 비웠어요");
}

function clearAllCrops() {
  if (!generatedCrops.length) return;
  if (!window.confirm("내 작업 보관함의 쉽게 자르기 파일도 모두 삭제됩니다. 계속할까요?")) return;
  generatedCrops.forEach((item) => URL.revokeObjectURL(item.url));
  generatedCrops = [];
  sessionCropIds.clear();
  clearWorksDb();
  renderWorks();
  setStatus("쉽게 자르기 작업을 모두 삭제했어요");
}
...
function getCropQuality(mimeType) {
  if (!exportQuality || mimeType === "image/png") return undefined;
  return clampNumber(exportQuality.value, 10, 100, 94) / 100;
}

function syncExportQualityUi() {
  if (!exportQuality || !exportQualityValue || !exportQualityRow) return;
  const isPng = formatSelect.value === "image/png";
  exportQualityValue.textContent = `${Math.round(clampNumber(exportQuality.value, 10, 100, 94))}%`;
  exportQuality.disabled = isPng;
  exportQualityRow.classList.toggle("is-disabled", isPng);
}

function rotatedBounds(rect, angle) {
  if (!angle) return { x: rect.x, y: rect.y, width: rect.width, height: rect.height };
...
function drawImageMappedToBounds(canvasContext, image, bounds, outputWidth, outputHeight) {
...
async function stampCrop() {
  ...
  const sampleBounds = activeShape === "freeform" ? bounds : rotatedBounds(bounds, crop.angle || 0);
  const target = getExportSize(sampleBounds.width, sampleBounds.height);
  ...
  if (activeShape === "freeform") {
    ...
    const scaleX = output.width / sampleBounds.width;
    const scaleY = output.height / sampleBounds.height;
    const shapeX = (bounds.x - sampleBounds.x) * scaleX;
    const shapeY = (bounds.y - sampleBounds.y) * scaleY;
    const shapeW = bounds.width * scaleX;
    const shapeH = bounds.height * scaleY;
    const ocx = shapeX + shapeW / 2;
    const ocy = shapeY + shapeH / 2;
    if (crop.angle) {
      outputCtx.translate(ocx, ocy);
      outputCtx.rotate(crop.angle);
      outputCtx.translate(-ocx, -ocy);
    }
    applyShapePath(outputCtx, activeShape, shapeX, shapeY, shapeW, shapeH);
    outputCtx.clip();
    if (crop.angle) {
      outputCtx.setTransform(1, 0, 0, 1, 0, 0);
    }
    drawImageMappedToBounds(outputCtx, sourceImage, sampleBounds, output.width, output.height);
  }
  outputCtx.restore();

  const blob = await canvasToBlob(output, mimeType, getCropQuality(mimeType));
...
function renderWorks() {
  ...
  const sessionOrdered = ordered.filter((item) => sessionCropIds.has(item.id));
  const has = ordered.length > 0;
  const hasSession = sessionOrdered.length > 0;

  if (resultGrid) {
    resultGrid.innerHTML = "";
    sessionOrdered.forEach((item) => resultGrid.append(buildWorkCard(item)));
  }
  if (worksGrid) {
    worksGrid.innerHTML = "";
    ordered.forEach((item) => worksGrid.append(buildWorkCard(item)));
  }

  if (resultEmpty) resultEmpty.classList.toggle("is-hidden", hasSession);
  if (worksEmpty) worksEmpty.classList.toggle("is-hidden", has);
  if (downloadAllBtn) downloadAllBtn.disabled = !hasSession;
  if (clearBtn) clearBtn.disabled = !hasSession;
  if (downloadAllWorks) downloadAllWorks.disabled = !has;
  if (clearWorks) clearWorks.disabled = !has;
  if (resultHint) {
    resultHint.textContent = hasSession ? `이번에 자른 파일 ${sessionOrdered.length}개` : "자른 파일이 여기에 바로 나타나요";
  }
  if (worksHint) {
    worksHint.textContent = has ? `지금까지 ${ordered.length}개의 작업을 만들었어요` : "지금까지 만든 파일이 여기에 모여요";
  }
  if (saveMeta) saveMeta.textContent = `${sessionOrdered.length}개`;
...
async function downloadZip(items = generatedCrops, baseName = "crops") {
  ...
  const link = document.createElement("a");
  link.href = url;
  link.download = `${baseName}_${formatDownloadStamp()}.zip`;
  document.body.appendChild(link);
  link.click();
  link.remove();
...
fileInput.addEventListener("change", (event) => {
  useFirstImageFile(event.target.files, "선택한 파일");
  fileInput.value = "";
});
...
dropzone.addEventListener("drop", (event) => useFirstImageFile(event.dataTransfer.files, "드롭한 파일"));

["dragenter", "dragover"].forEach((eventName) => {
  stageWrap.addEventListener(eventName, (event) => {
    event.preventDefault();
    stageWrap.classList.add("is-dragover");
  });
});
["dragleave", "drop"].forEach((eventName) => {
  stageWrap.addEventListener(eventName, (event) => {
    event.preventDefault();
    stageWrap.classList.remove("is-dragover");
  });
});
stageWrap.addEventListener("drop", (event) => useFirstImageFile(event.dataTransfer.files, "드롭한 파일"));

document.addEventListener("paste", (event) => {
  if (currentViewName !== "edit" || isTypingTarget(event.target)) return;
  const files = Array.from(event.clipboardData?.files || []).filter((file) => file.type.startsWith("image/"));
  if (!files.length) return;
  event.preventDefault();
  useFirstImageFile(files, "클립보드 이미지");
});
...
function clearCurrentSelection(message = "선택을 취소했어요") {
  if (!cropActive && !freeformPoints.length && !pendingDraw && !isDrawingShape) return false;
  cropActive = false;
  isDrawingShape = false;
  pendingDraw = false;
  freeformPoints = [];
...
document.addEventListener("keydown", (event) => {
  ...
  if (event.key === "Escape" || event.key === "Delete") {
    if (clearCurrentSelection()) event.preventDefault();
    return;
  }

  if (event.key === "Enter") {
    if ((activeShape === "freeform" && freeformPoints.length > 2) || (activeShape !== "freeform" && cropActive)) {
      stampCrop();
      event.preventDefault();
    }
    return;
...
function getExportBaseBounds() {
  const bounds = getCurrentBounds();
  if (!bounds) return null;
  return activeShape === "freeform" ? bounds : rotatedBounds(bounds, crop.angle || 0);
}

function syncCustomExportRatio(changed) {
  if (syncingExportCustomSize || exportMode !== "custom" || !exportRatioLock?.checked) return;
  const bounds = getExportBaseBounds();
  if (!bounds || !bounds.width || !bounds.height) return;
  syncingExportCustomSize = true;
  const ratio = bounds.width / bounds.height;
  if (changed === "width") {
    const width = clampNumber(exportWidthInput.value, 1, 20000, bounds.width);
    exportHeightInput.value = Math.max(1, Math.round(width / ratio));
  } else {
    const height = clampNumber(exportHeightInput.value, 1, 20000, bounds.height);
    exportWidthInput.value = Math.max(1, Math.round(height * ratio));
  }
  syncingExportCustomSize = false;
}

function updateExportInfo() {
  const bounds = getCurrentBounds();
  const exportBounds = getExportBaseBounds();
  if (exportCurrent) {
  ...
  if (exportPreview) {
    if (!exportBounds) {
      ...
      const target = getExportSize(exportBounds.width, exportBounds.height);
      exportPreview.textContent = `저장 크기 ${target.width} × ${target.height}px`;
    }
  }
  if (exportRatioWarning) {
    let warn = false;
    if (exportMode === "custom" && exportBounds) {
      const target = getExportSize(exportBounds.width, exportBounds.height);
      const sourceRatio = exportBounds.width / exportBounds.height;
      const targetRatio = target.width / target.height;
      warn = Math.abs(sourceRatio - targetRatio) > sourceRatio * 0.01;
    }
    exportRatioWarning.hidden = !warn;
  }
}
...
exportScaleInput.addEventListener("input", updateExportInfo);
exportWidthInput.addEventListener("input", () => {
  syncCustomExportRatio("width");
  updateExportInfo();
});
exportHeightInput.addEventListener("input", () => {
  syncCustomExportRatio("height");
  updateExportInfo();
});
if (exportRatioLock) {
  exportRatioLock.addEventListener("change", () => {
    if (exportRatioLock.checked) syncCustomExportRatio("width");
    updateExportInfo();
  });
}
if (formatSelect) {
  formatSelect.addEventListener("change", syncExportQualityUi);
}
if (exportQuality) {
...
function transformSource(kind) {
  if (!sourceImage) return;
  const hadSelection = cropActive || freeformPoints.length > 0;
  ...
  newImage.onload = () => {
    ...
    drawStage();
    commitHistory();
    const label = TRANSFORM_LABELS[kind] || "이미지를 변형했어요";
    setStatus(hadSelection ? `${label} 선택 영역은 초기화됐어요` : label);
  };
...
async function addBatchSources(files) {
  ...
  const excluded = Math.max(0, imageFiles.length - selected.length);
  if (!selected.length) {
  ...
  drawBatchPreview();
  syncBatchControls();
  setBatchStatus(excluded ? `${batchSources.length}개 원본 이미지 준비됨 · 초과분 ${excluded}개 제외` : `${batchSources.length}개 원본 이미지 준비됨`);
}
...
function drawBatchPreview() {
  resizeCanvasBufferToDisplaySize(batchPreviewCanvas);
  const context = batchPreviewCanvas.getContext("2d");
  const dpr = Number(batchPreviewCanvas.dataset.pixelRatio || 1);
  const canvasWidth = batchPreviewCanvas.width / dpr;
  const canvasHeight = batchPreviewCanvas.height / dpr;
...
async function downloadBatchZip() {
  ...
  const link = document.createElement("a");
  link.href = url;
  link.download = `batch-paste_${formatDownloadStamp()}.zip`;
  document.body.appendChild(link);
  link.click();
  link.remove();
  URL.revokeObjectURL(url);
...
const batchCropResetOne = document.getElementById("batchCropResetOne");
const batchCropCopyToSelected = document.getElementById("batchCropCopyToSelected");
...
function resetCurrentBatchCropRect() {
  const source = currentBatchCropSource();
  if (!source || batchCropRunning) return;
  source.rect = { x: 0.1, y: 0.1, width: 0.8, height: 0.8 };
  batchCropRect = source.rect;
  constrainBatchCropRect();
  drawBatchCropPreview();
  renderBatchCropSources();
  setBatchCropStatus(`${source.file.name} 영역을 초기화했어요`);
}

function copyCurrentBatchCropToSelected() {
  const source = currentBatchCropSource();
  if (!source || batchCropRunning) return;
  const selected = batchCropSources.filter((item) => item.selected && item !== source);
  selected.forEach((item) => {
    item.rect = { ...source.rect };
  });
  drawBatchCropPreview();
  renderBatchCropSources();
  setBatchCropStatus(selected.length ? `현재 영역을 선택한 사진 ${selected.length}개에 복사했어요` : "복사할 선택된 다른 사진이 없어요");
}
...
async function addBatchCropSources(files) {
  ...
  setBatchCropStatus(excluded ? `${batchCropSources.length}개 원본 이미지 준비됨 · 초과분 ${excluded}개 제외` : `${batchCropSources.length}개 원본 이미지 준비됨`);
}
...
function drawBatchCropPreview() {
  resizeCanvasBufferToDisplaySize(batchCropPreviewCanvas);
  const context = batchCropPreviewCanvas.getContext("2d");
  const dpr = Number(batchCropPreviewCanvas.dataset.pixelRatio || 1);
  const canvasWidth = batchCropPreviewCanvas.width / dpr;
  const canvasHeight = batchCropPreviewCanvas.height / dpr;
  context.setTransform(dpr, 0, 0, dpr, 0, 0);
  context.clearRect(0, 0, canvasWidth, canvasHeight);
  batchCropFrame = null;
  const source = currentBatchCropSource();
...
function syncBatchCropControls() {
  ...
  [batchCropResetOne, batchCropCopyToSelected].forEach((control) => {
    if (control) control.disabled = !ready || batchCropRunning;
  });
}
...
async function downloadBatchCropZip() {
  ...
  downloadBlobAs(makeZip(files), `batch-crop_${formatDownloadStamp()}.zip`);
...
if (batchCropResetOne) batchCropResetOne.addEventListener("click", resetCurrentBatchCropRect);
if (batchCropCopyToSelected) batchCropCopyToSelected.addEventListener("click", copyCurrentBatchCropToSelected);
...
function setView(name, navButton) {
  ...
  if (name === "bgremove") requestAnimationFrame(drawBgPreview);
}
...
downloadAllBtn.addEventListener("click", () => downloadZip(getSessionCrops(), "crops"));
clearBtn.addEventListener("click", resetResults);
downloadAllWorks.addEventListener("click", () => downloadZip(generatedCrops, "crops"));
clearWorks.addEventListener("click", clearAllCrops);
...
function clearRenameWorksEntries() {
  ...
  if (!window.confirm("파일 일괄 편집 탭의 현재 목록도 함께 비워집니다. 계속할까요?")) return;
  renameEntries.forEach((entry) => {
...
async function downloadRenameZip() {
  ...
  downloadBlobAs(makeZip(files), `renamed_${formatDownloadStamp()}.zip`);
...
async function downloadCompressZip() {
  ...
  downloadBlobAs(makeZip(files), `compressed_${formatDownloadStamp()}.zip`);
  setRenameStatus(`${files.length}개 파일을 ZIP으로 저장했어요`);
}
...
const bgResetSettings = document.getElementById("bgResetSettings");
const bgApplyOne = document.getElementById("bgApplyOne");
const bgApplyAll = document.getElementById("bgApplyAll");
const bgResultGrid = document.getElementById("bgResultGrid");
const bgResultEmpty = document.getElementById("bgResultEmpty");
const bgResultCount = document.getElementById("bgResultCount");
const bgResultHint = document.getElementById("bgResultHint");
const bgPrefix = document.getElementById("bgPrefix");
...
async function addBgFiles(fileList) {
  if (bgRunning) return;
  const available = Math.max(0, 50 - bgEntries.length);
  const files = Array.from(fileList || []).filter((file) => file.type.startsWith("image/"));
  const selected = files.slice(0, available);
  const excluded = Math.max(0, files.length - selected.length);
  if (!selected.length) {
    setBgStatus(available ? "사용할 수 있는 이미지 파일이 없어요" : "이미지는 최대 50개까지 올릴 수 있어요");
    return;
  }
  ...
  if (bgSelectedIndex < 0 && bgEntries.length) bgSelectedIndex = firstNew;
  renderBgSources();
  scheduleBgPreview();
  syncBgControls();
...
function removeBgEntry(index) {
  ...
  scheduleBgPreview();
  syncBgControls();
  setBgStatus(bgEntries.length ? `${bgEntries.length}개 이미지` : "이미지를 업로드하세요");
}

function renderBgSources() {
  if (!bgSourceGrid) return;
  bgSourceGrid.innerHTML = "";
  bgSourceGrid.classList.toggle("is-list", bgEntries.length > 9);
  bgEntries.forEach((entry, index) => {
    const card = document.createElement("article");
    card.className = `batch-file-card bg-file-card${index === bgSelectedIndex ? " is-active" : ""}`;
    card.tabIndex = 0;
    card.setAttribute("role", "button");
    card.setAttribute("aria-label", `${entry.originalName} 미리보기`);
    const image = document.createElement("img");
    image.src = entry.url;
    image.alt = "";
    const name = document.createElement("span");
    ...
    remove.className = "batch-file-remove";
    remove.textContent = "×";
    remove.setAttribute("aria-label", `${entry.originalName} 제거`);
    remove.addEventListener("click", (event) => {
      event.stopPropagation();
      if (!bgRunning) removeBgEntry(index);
    });
    const select = () => {
      if (bgRunning) return;
    ...
    if (entry.done) {
      ...
      badge.className = "bg-done-badge";
      badge.textContent = "✓";
      badge.title = "적용됨";
      card.append(badge);
    }
    card.append(image, name, remove);
    bgSourceGrid.append(card);
  });
...
function bgBoxBlur(src, w, h, r) {
  ...
  for (let y = 0; y < h; y++) {
    const row = y * w;
    let acc = 0;
    for (let k = -r; k <= r; k++) acc += src[row + clampX(k)];
    for (let x = 0; x < w; x++) {
      tmp[row + x] = acc * norm;
...
function scheduleBgPreview() {
  if (bgPreviewRaf) return;
  bgPreviewRaf = requestAnimationFrame(() => {
    bgPreviewRaf = 0;
    drawBgPreview();
  });
}
...
function drawBgPreview() {
  ...
  if (bgPreviewCanvas.width !== cw) bgPreviewCanvas.width = cw;
...
function bgOutName(entry) {
  const prefix = safeRenameBase(bgPrefix ? bgPrefix.value : "nobg", "nobg");
  const serial = String(bgResults.length + 1).padStart(3, "0");
  return `${prefix}_${serial}_${safeRenameBase(entry.base, entry.base)}.png`;
}

async function runBgRemove(entriesToProcess) {
  if (bgRunning || !entriesToProcess.length) return;
  bgRunning = true;
  syncBgControls();
  renderBgSources();
  if (bgProgress) {
    bgProgress.hidden = entriesToProcess.length < 2;
    bgProgressBar.max = entriesToProcess.length;
    bgProgressBar.value = 0;
    bgProgressText.textContent = `0 / ${entriesToProcess.length}`;
  }
  let completed = 0;
  let failed = 0;
  for (const entry of entriesToProcess) {
    try {
      ...
      entry.done = true;
      completed += 1;
      if (bgAutoDownload && bgAutoDownload.checked) downloadBlobAs(item.blob, item.file);
    } catch (error) {
      failed += 1;
    }
    if (bgProgress && !bgProgress.hidden) {
      bgProgressBar.value = completed + failed;
      bgProgressText.textContent = `${completed + failed} / ${entriesToProcess.length}`;
    }
    renderBgResults();
  }
  bgRunning = false;
...
function buildBgResultCard(item) {
  ...
  remove.addEventListener("click", () => {
    if (bgRunning) return;
    const index = bgResults.findIndex((result) => result.id === item.id);
    if (index === -1) return;
    URL.revokeObjectURL(bgResults[index].url);
    bgResults.splice(index, 1);
    renderBgResults();
  });
  actions.append(link, remove);
  footer.append(name, dim, actions);
...
function renderBgResults() {
  if (!bgResultGrid) return;
  bgResultGrid.innerHTML = "";
  bgResults.slice().reverse().forEach((item) => bgResultGrid.append(buildBgResultCard(item)));
  const hasResults = bgResults.length > 0;
  if (bgResultEmpty) bgResultEmpty.classList.toggle("is-hidden", hasResults);
  if (bgResultCount) bgResultCount.textContent = `${bgResults.length}개`;
  if (bgResultHint) bgResultHint.textContent = hasResults ? `배경 지운 파일 ${bgResults.length}개 · 투명 PNG` : "적용한 파일이 여기에 표시됩니다 · 투명 PNG로 저장돼요";
  syncBgControls();
}

function clearBgResults() {
  if (bgRunning || !bgResults.length) return;
  bgResults.forEach((item) => URL.revokeObjectURL(item.url));
  bgResults = [];
  bgEntries.forEach((entry) => (entry.done = false));
  renderBgResults();
  renderBgSources();
  setBgStatus("결과를 비웠어요");
}

async function downloadBgZip() {
  if (!bgResults.length) return;
  setBgStatus("ZIP 생성 중…");
  const used = new Map();
  const files = [];
  for (const item of bgResults) {
    let name = item.file;
    if (used.has(name)) {
      const count = used.get(name) + 1;
      used.set(name, count);
    ...
    files.push({ name, bytes: new Uint8Array(await item.blob.arrayBuffer()) });
  }
  downloadBlobAs(makeZip(files), `background-removed_${formatDownloadStamp()}.zip`);
  setBgStatus(`${files.length}개 파일을 ZIP으로 저장했어요`);
}

if (bgSourceInput) {
  bgSourceInput.addEventListener("change", (event) => {
    addBgFiles(event.target.files);
    bgSourceInput.value = "";
  });
  ...
  bgFillSeg.querySelectorAll(".seg-btn").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      bgFillSeg.querySelectorAll(".seg-btn").forEach((b) => b.classList.toggle("is-active", b === button));
      if (bgFillColorLine) bgFillColorLine.hidden = bgSettings.fill !== "custom";
      scheduleBgPreview();
    });
  });
  ...
  if (bgFillColor) {
    bgFillColor.addEventListener("input", () => {
      bgSettings.fillColor = bgFillColor.value;
      if (bgSettings.fill === "custom") scheduleBgPreview();
    });
  }
...
const HOWTO = {
  ...
  bgremove: {
    ...
    svg: `<svg viewBox="0 0 320 190" class="ho-svg" role="img" aria-label="배경 지우기 미리보기">
      ${HOWTO_PHOTO_DEFS}
      <rect x="26" y="24" width="128" height="142" rx="12" fill="url(#hoPhoto)" />
      ${howtoScenery(26, 24, 128, 142)}
      <circle cx="90" cy="92" r="26" fill="#fca5a5" />
      <g class="ho-cursor" style="--x:70px"><path d="M0 0 L0 19 L5 14 L9 22 L13 20 L9 13 L16 13 Z" fill="#1f2330" stroke="#fff" stroke-width="1.3" /></g>
      <g class="ho-result">
        <rect x="190" y="40" width="108" height="110" rx="12" fill="#ffffff" />
        <g fill="#e7e9f2">
          <rect x="190" y="40" width="18" height="18" /><rect x="226" y="40" width="18" height="18" /><rect x="262" y="40" width="18" height="18" />
          <rect x="208" y="58" width="18" height="18" /><rect x="244" y="58" width="18" height="18" /><rect x="280" y="58" width="18" height="18" />
          <rect x="190" y="76" width="18" height="18" /><rect x="226" y="76" width="18" height="18" /><rect x="262" y="76" width="18" height="18" />
          <rect x="208" y="94" width="18" height="18" /><rect x="244" y="94" width="18" height="18" /><rect x="280" y="94" width="18" height="18" />
          <rect x="190" y="112" width="18" height="18" /><rect x="226" y="112" width="18" height="18" /><rect x="262" y="112" width="18" height="18" />
          <rect x="208" y="130" width="18" height="18" /><rect x="244" y="130" width="18" height="18" /><rect x="280" y="130" width="18" height="18" />
        </g>
        <circle cx="244" cy="95" r="26" fill="#fca5a5" />
      </g>
    </svg>`,
  },
};
...
function openHowto() {
  ...
  requestAnimationFrame(() => howtoClose?.focus());
}
...
document.addEventListener("keydown", (event) => {
  ...
  const focusable = Array.from(howtoModal.querySelectorAll("button, [href], input, select, textarea, [tabindex]:not([tabindex='-1'])"))
